...
# source: https://github.com/yinhm/datafeed/blob/master/datafeed/providers/dzh.py
"""大智慧日线数据抓换
...
class EndOfIndexError(Exception):
    pass
...
def gb2utf(value, ignore=True):
    if ignore:
        return str(value, 'gb18030', 'ignore').encode('utf-8', 'ignore')
    else:
        return str(value, 'gb18030').encode('utf-8')
...
class DzhFetcher(object):
    ...
    def fetch(self):
        try:
            r = urllib.request.urlopen(self.data_url())
            data = r.read()
